...
In this first part of the tutorial we will set up the graphical user interface of FragPipe and launch a library-free search combining MSFragger-DIA and DIA-NN. The end result will be the generation of a collection matrices with the quantification values at the precursor and protein levels, as well as a summary pdf file of the experiment. Here we use FragPipe 20.1-build15 as an example:
