...
- `sample_name` is the same as `sample` (`experiment`) when FragPipe first generated it, but user could edit this as a way to update the visualization. Values in the `sample_name` also couldn't be simply digits like `123` or `456` or start with digits.
- `replicate` will be automatically propagated based on what users specified in FragPipe.
Note that since FragPipe-Analyst focus on differential expression analysis. You should have more than one condition available in the `condition` column like T/N or Tumor/NAT in the examples mentioned above. Each condition should have at least two samples.
